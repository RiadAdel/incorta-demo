...
kpi_customers  = df["CustomerID"].nunique()
kpi_total      = df["Total_Sales"].sum()
kpi_avg_sales  = df["Total_Sales"].mean()
kpi_avg_orders = df["Sales_Order_Count"].mean()
...
with btn_col:
    with st.popover("⬇  Download", use_container_width=True):
        st.download_button(
            "📄  PDF Report",
            data=build_pdf(df, kpi_customers, kpi_total, kpi_avg_sales, kpi_avg_orders),
        ...
        st.download_button(
            "📊  PowerPoint",
            data=build_pptx(df, kpi_customers, kpi_total, kpi_avg_sales, kpi_avg_orders),
...
# ── Row 1: Top Cities | Sales vs Orders ──────────────────────────────────────
left, right = st.columns(2)

with left:
    st.subheader("Top 10 Cities by Sales")
    city_df = df.groupby("City", as_index=False)["Total_Sales"].sum().nlargest(10, "Total_Sales")
    fig_city = px.bar(city_df, x="Total_Sales", y="City", orientation="h",
...
with right:
    ...
    fig_scatter = px.scatter(df, x="Sales_Order_Count", y="Total_Sales",
...
# ── Row 2: Avg Order Value | Top States ──────────────────────────────────────
...
with left2:
    ...
    fig_avg = px.bar(avg_df, x="Avg Order Value", y="State", orientation="h",
...
with right2:
    ...
    fig_state = px.bar(state_sales_df, x="Total Sales", y="State", orientation="h",
...
st.dataframe(
    ...
    use_container_width=True,
    column_config={
        "AccountNumber":       st.column_config.TextColumn("Account #"),
        "CustomerID":          st.column_config.TextColumn("Customer ID"),
        "City":                st.column_config.TextColumn("City"),
        "StateProvinceName":   st.column_config.TextColumn("State / Province"),
        "Sales_Order_Count":   st.column_config.NumberColumn("Orders", format="%d"),
        "Average_Sales_Order": st.column_config.NumberColumn("Avg Order Value", format="$%.0f"),
